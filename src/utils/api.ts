...
let API_ORIGIN = API_BASE_URL.replace(/\/api$/, '');
// If site is served over HTTPS but API origin is HTTP, upgrade to HTTPS to avoid mixed content
try {
  if (typeof window !== 'undefined' && window.location?.protocol === 'https:') {
    const u = new URL(API_ORIGIN);
    if (u.protocol === 'http:') {
      u.protocol = 'https:';
      API_ORIGIN = u.toString().replace(/\/$/, '');
    }
  }
} catch {}
